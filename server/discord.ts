 ...
 * This matches the comprehensive detection in routes.ts
 */
function isComprehensiveDirectQuestion(message: string): boolean {
    ...
    this.client.on(Events.MessageCreate, async (message: Message) => {
      try {
        ...
        // Priority 4: Even when not in streaming mode, respond to direct questions and greetings
          ...
          const content = message.content.toLowerCase();
          // Use the same comprehensive direct question detection as the backend
          const isDirectQuestion = isComprehensiveDirectQuestion(message.content);
          ...
          if (isDirectQuestion || isDirectGreeting) {
            ...
            if (isDirectQuestion) {
              console.log(`Discord: Direct question detected: "${message.content}" - setting responseReason to 'direct interaction'`);
            }
          }
        }
